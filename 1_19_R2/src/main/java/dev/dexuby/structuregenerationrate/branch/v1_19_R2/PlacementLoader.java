package dev.dexuby.structuregenerationrate.branch.v1_19_R2;
...
import dev.dexuby.structuregenerationrate.branch.v1_19_R2.config.ConcentricRingsStructureConfiguration;
import dev.dexuby.structuregenerationrate.branch.v1_19_R2.config.RandomSpreadStructureConfiguration;
            ...
            for (Map.Entry<ResourceKey<StructureSet>, Holder.Reference<StructureSet>> entry : new ArrayList<>(byKeyRef.get().entrySet())) {
                ...
                if (pl.getConfiguration().doCacheDefaultValues() && this.initial) {
                    if (!pl.getCachedDefaultValues().containsKey(name))
                        pl.getCachedDefaultValues().put(name, (isrsp)? RandomSpreadStructureConfiguration.fromRandomSpreadStructurePlacement(rsplacement) : ConcentricRingsStructureConfiguration.fromConcentricRingsStructurePlacement(crplacement));
                }
