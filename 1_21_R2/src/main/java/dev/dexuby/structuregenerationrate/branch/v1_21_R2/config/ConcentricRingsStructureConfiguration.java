package dev.dexuby.structuregenerationrate.branch.v1_21_R2.config;
...
public final class ConcentricRingsStructureConfiguration implements IStructureConfiguration {
    ...
    public static String[] holderSetBiomesToArray(HolderSet<Biome> biomesSet) {
        ...
        biomesSet.forEach(h ->{
            if (!h.isBound()) return;
            for (Map.Entry<ResourceKey<Biome>, Biome> e : ((CraftServer) Bukkit.getServer()).getServer().registryAccess().lookupOrThrow(Registries.BIOME).entrySet()) {
