package dev.dexuby.structuregenerationrate.branch.v1_19_R1.config;
...
public final class ConcentricRingsStructureConfiguration implements IStructureConfiguration {
    ...
    public static String[] holderSetBiomesToArray(HolderSet<Biome> biomesSet) {
        ...
        biomesSet.forEach(h ->{
            if (!h.isBound()) return;
            for (Map.Entry<ResourceKey<Biome>, Biome> e : ((CraftServer) Bukkit.getServer()).getServer().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).entrySet()) {
