package dev.dexuby.structuregenerationrate.branch.v1_19_R1;
...
import dev.dexuby.structuregenerationrate.branch.v1_19_R1.config.ConcentricRingsStructureConfiguration;
import dev.dexuby.structuregenerationrate.branch.v1_19_R1.config.RandomSpreadStructureConfiguration;
...
public class PlacementLoader implements IPlacementLoader {
    ...
    public void load() {
        ...
        for (final World world : Bukkit.getServer().getWorlds()) {
            ...
            final ReflectionUtils.FieldReference<Boolean> frozenRef = ReflectionUtils.getDeclaredField(MappedRegistry.class, setRegistry, "ca"); //frozen
            if (initial) {
                try {
                    frozenRef.set(false);
            ...
            final Map<ResourceKey<StructureSet>, Holder.Reference<StructureSet>> byKey = ReflectionUtils.getDeclaredField(MappedRegistry.class, setRegistry, "bV").cast(Map.class); // byKey
            for (Map.Entry<ResourceKey<StructureSet>, Holder.Reference<StructureSet>> entry : new ArrayList<>(byKey.entrySet())) {
                ...
                if (isrsp) {
                    ...
                    oldPlacement = new ConcentricRingsStructurePlacement(1,1,1, HolderSet.direct());
                ...
                final ServerChunkCache chunkCache = ((CraftWorld) world).getHandle().getChunkSource();
                final ChunkGenerator generator = chunkCache.getGenerator();
                ...
                ReflectionUtils.getDeclaredField(ChunkGenerator.class, generator, "h").cast(Map.class).clear(); // placementsForStructure
                ReflectionUtils.getDeclaredField(ChunkGenerator.class, generator, "i").cast(Map.class).clear(); // ringPositions

                // All positions are regenerated to apply the new configurations
                ReflectionUtils.callMethod(ChunkGenerator.class, generator, "b", ReflectionUtils.param(RandomState.class, chunkCache.randomState())); // generatePositions(RandomState)
    ...
    public IStructureConfiguration toStructureConfiguration(String structure, String path, Configuration config) {
        if (structure.equals("strongholds")) {
            ...
            HolderSet<Biome> biomeSet = preferredBiomes.isEmpty()? HolderSet.direct() : ConcentricRingsStructureConfiguration.biomeListToHolderSet(preferredBiomes);
